...
def setup(
        input_file_lines: List[str]) -> Tuple[Dict[complex, str], List[Cart]]:
    ...
    for y, line in enumerate(input_file_lines):
        for x, char in enumerate(line):
            ...
            if char in "<v>^":
                ...
                carts.append(Cart(x + y * 1j,
                                  direction))  # location, direction, crossings
                part = {
                    "<": "-",
                    "v": "|",
                    ">": "-",
...
def solve_a(input_file_lines: List[str]) -> str:
    ...
    while True:
        ...
        for ci, cart in enumerate(carts):
            ...
            if any(c2.position == cart.position for c2i, c2 in enumerate(carts)
                   if c2i != ci):
                return str(int(cart.position.real)) + "," + str(
                    int(cart.position.imag))
                # 14, 42
